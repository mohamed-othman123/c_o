...
export type WidgetStatus = 'empty' | 'loading' | 'error' | 'default';

@Component({
  selector: 'dashboard-widget',
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [Icon, Card, Button, CommonModule, TranslocoDirective, NgClass],
  template: `
    <scb-card
      class="py-xl flex h-full flex-col rounded-4xl !px-0"
      *transloco="let t; prefix: 'dashboard'">
      <div class="mb-lg gap-md px-xl flex items-center">
        <div
          class="gap-sm flex flex-1 flex-col"
          [ngClass]="verticalTitle() ? '' : '2xl:flex-row 2xl:items-center 2xl:justify-between'">
          <div>
            <h4 class="head-xs-s flex-1"><ng-content select=".title" /></h4>
            <ng-content select=".sub-title" />
          </div>
          @if (status() === 'default') {
            <ng-content select=".lastUpdated"></ng-content>
          }
        </div>
        @if (status() === 'default') {
          <ng-content select=".showReload"></ng-content>
          <ng-content select=".seeAll"></ng-content>
        }
      </div>
...
export class DashboardWidget {
  readonly status = input<WidgetStatus>('loading');
